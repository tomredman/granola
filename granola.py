...
parser = argparse.ArgumentParser(description='Start a repository server and interact with the TC.')
    ...
    def _execute_operation(self, operation, filename, data):
        ...
        if operation == 'create' or operation == 'update':
            try:
                ...
                self.file_metadata[filename] = time.time()  # Update file's last modification timestamp
...
def prepare():
    ...
    if repo.prepare_transaction(tid, operation, filename, data_content):
        return jsonify({"status": "prepared", "tid": tid, "operation": operation, "filename": filename, "data": data_content})
    else:
...
def register_with_coordinator():
    ...
    response = requests.post(tc_url, json={"repo_name": args.name, "repo_url": repo_url})
    if response.status_code == 200:
        print(f"Successfully registered {args.name} with the TC.")
    else:
        print(f"Failed to register {args.name} with the TC. Status code: {response.status_code}")
