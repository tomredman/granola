...
if __name__ == "__main__":
    ...
    for i in range(500):
        ...
        if should_sleep == 1:
            ...
            logger.info(Fore.YELLOW + 'Sleeping for ' + str(sleep_time) + 's' + Style.RESET_ALL)
        ...
        filename = args.file #this is the file that the transaction will be performed on, or the resource a user is "editing"
        indep_result = client.invokeIndep(["repo1", "repo2", "repo3", "repo4"], {"operation": "create", "filename": filename, "data": {"tid": tid, "autoKey"+str(i): random_value}})

        logger.info(Fore.GREEN + 'Independent transaction result ' + str(i) + ' for file ' + filename + Style.RESET_ALL)
