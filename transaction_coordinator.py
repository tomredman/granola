...
class TransactionCoordinator:
    ...
    def start_transaction(self, repository_names, transaction_details, read_only=False):
        ...
        repository_urls = [self.get_repository_url(name) for name in repository_names if self.get_repository_url(name)]
        execute_payload = {'tid': tid, 'filename': filename, **transaction_details}

        # First, attempt to prepare the transaction at each repository
        preparation_results = [requests.post(f"{url}/prepare", json=execute_payload).json() for url in repository_urls]
        prepared = all(result.get('status') == 'prepared' for result in preparation_results)
        ...
        execution_results = [requests.post(f"{url}/{action}", json={'tid': tid}).json() for url in repository_urls]
        success = all(result.get('status') == 'committed' for result in execution_results)

        if not success:
            errors = [result for result in execution_results if result.get('status') != 'committed']
            # todo: execute rollback!
            return {"status": "error", "transaction": tid, "action": action, "message": "Transaction failed, rolling back.", "errors": errors}
...
def invoke_single_transaction():
    ...
    repository_urls = [tc.get_repository_url(name) for name in repository_names if tc.get_repository_url(name)]
    if repository_urls:
